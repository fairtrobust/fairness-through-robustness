...
#                                        labels=labels, kwargs=self.attack_call_kwargs)
            ...
            if len(adversarial_images) == 0:
                print(f'No adversarial images found in epoch {idx+1}')
                continue
...
#             all_tups = pool.starmap(partial(self.attack(**kwargs)), args)
    ...
    def plot_example(self, stacked_image, labels, predictions_on_attacked):
        if not os.path.exists('./models/{}/adversarial_examples/{}/{}.png'.format(self.ds.name, self.model.model_name,
                                                                      '{}_adv_example'.format(self.name))):
            hp.plot_image(stacked_image, subfolder=self.model.model_name, ds=self.ds.name,
    ...
    def get_adversarial_image(self, image_id, epoch, post_fix):
        ...
        """
        if not os.path.exists("{}/models/{}/adversarial_images/{}/{}".format(self.root_dir, self.ds.name,
                                                                         self.model.model_name, self.name)):
            return False, None
        else:
            if not os.path.exists("{}/models/{}/adversarial_images/{}/{}/{}_epoch_{}{}.pkl".format(self.root_dir,
                                                    self.ds.name, self.model.model_name, self.name, 
                                                    int(image_id), epoch, post_fix)):
                return False, None
            with open("{}/models/{}/adversarial_images/{}/{}/{}_epoch_{}{}.pkl".format(self.root_dir,
                                                    self.ds.name, self.model.model_name, self.name, 
...
#             all_tups = pool.starmap(partial(self.attack(**kwargs)), args)
    ...
    def get_adversarial_image(self, image_id, epoch, post_fix):
        ...
        checks on disk, if found, return the adversarial object
        """
        if not os.path.exists("{}/models/{}/adversarial_images/{}_{}/{}".format(self.root_dir, self.ds.name,
            self.model.model_name, self.model.criterion._get_name(), self.name)):
            return False, None
        else:
            if not os.path.exists("{}/models/{}/adversarial_images/{}_{}/{}/{}_epoch_{}{}.pkl".format(self.root_dir,
                                self.ds.name, self.model.model_name, self.model.criterion._get_name(), 
                                self.name, int(image_id), epoch, post_fix)):
                return False, None
            with open("{}/models/{}/adversarial_images/{}_{}/{}/{}_epoch_{}{}.pkl".format(self.root_dir,
