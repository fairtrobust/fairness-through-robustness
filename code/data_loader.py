...
def plot_image(image, root_folder='.', ds='unnamed', subfolder='unnamed', filename='unnamed',
    ...
    if save_fig:
        ...
        hp.create_dir(base_dir)
        hp.create_dir('{}/{}'.format(base_dir, subfolder))
        plt.savefig('{}/{}/{}.{}'.format(base_dir, subfolder, filename, extension))
    if show_fig:
        plt.show()
    plt.clf()
    plt.close()
...
#         self.race_id_to_label = {0: 'other', 1: 'black'}
    ...
    def visual_samples(self):
        ...
        for i in range(5):
            ...
            plot_image(train_image, save_fig=False, ds=self.name,
        ...
        for i in range(5):
            ...
            plot_image(test_image, save_fig=False, ds=self.name,
...
class CIFAR10(Dataloader):
    ...
    def __init__(self, name, root_dir='.'):
        ...
        cifar10_train = torchvision.datasets.CIFAR10(root='{}/data'.format(root_dir), train=True,
                                                      download=True, transform=self.data_transform)
        cifar10_test = torchvision.datasets.CIFAR10(root='{}/data'.format(root_dir), train=False,
                                                      download=True, transform=self.data_transform)
