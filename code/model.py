...
from custom_models import MLP1, Conv2_CIFAR, DeepCNN, UTKClassifier, AdienceClassifier
from regularized_loss import RegularizedLoss
import helper as hp

# These variables were undefined here, and were defined in experiment-main.py
PHASES = ['train', 'test']
...
def train_model(model, num_epochs, device, ds_name, data_loaders, criterion_kwargs={}, checkpoint=None):
    ...
    if checkpoint is not None:
        ...
        filename = '{}_{}_epoch_{}_lr_{}.pth'.format(model.model_name, model.criterion._get_name(), checkpoint,
                                                     learning_rate)
        model.model_ft.load_state_dict(torch.load('./models/{}/model_weights/{}'.format(ds_name, filename),
                                                 map_location=device))
    ...
    for epoch in iterator:
        ...
        for phase in ['train', 'test']:
            ...
            for _, inputs, labels, protected_classes in data_loaders[phase]:
                ...
                if ds_name.lower() == 'cifar10' and not ('mlp1' in model.model_name.lower() or 'deep_cnn' in model.model_name.lower()):
...
def load_model_history(model, ds_obj, num_epochs, portion, device, override_criterion=None, criterion_kwargs={}):
    ...
    for epoch in np.arange(0, num_epochs, model.aggregate_coeff):
        weights_filename = './models/{}/model_weights/{}_{}_epoch_{}_lr_{}.pth'.format(ds_obj.name, model.model_name, model.criterion._get_name(), epoch, learning_rate)
